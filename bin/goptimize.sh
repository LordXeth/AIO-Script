...
################################################################
 OPTIONS:                                                      #
  ...
  -p   Optimize PNG's                                          #
  -9   Skip 9-patch (*.9.png) PNG's while optimizing PNG's     #
  ...
  -b   Remove debug info from classes.dex for API[1-19] with   #
				...
				if [ "$opt_b" ]; then
					if java -version &>/dev/null && [ "$(java -version 2>&1) | grep 'java version')" ]; then
						...
						for SMALI in $SMALI_LIST; do
							...
							if [ $i -eq 0 ]; then
								echo -n " |- Removing debug info with smali$($SMALI --version | sed -ne 's@^\(bak\)\?smali \([0-9a-f\.\-]\+\)\(-dirty\)\? .*$@ v\2@Ip'): "
							else
								echo -n " |  +- Trying to remove with smali$($SMALI --version | sed -ne 's@^\(bak\)\?smali \([0-9a-f\.\-]\+\)\(-dirty\)\? .*$@ v\2@Ip'): "
							fi
